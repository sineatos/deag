...
func newDECurrentToRand1Statistics() support.Statistics {
...
func newDECurrentToRand1Constraint(low, up float64, evaluator benchmarks.Float64Evaluator) func(*base.Float64Individual) []float64 {
...
func newDECurrentToRand1Population(size, dims int, low, up float64) base.Individuals {
...
func TestDECurrentToRand1(t *testing.T) {
	f := 0.5
	maxGen, maxFES := 200, math.MaxInt64
	st, hof := newDECurrentToRand1Statistics(), support.NewDefaultHallOfFame(3, nil)
	size, dims := 200, 5
	low, up := -5.12, 5.12
	pop := newDECurrentToRand1Population(size, dims, low, up)
	evaluator := newDECurrentToRand1Constraint(low, up, benchmarks.Rastrigin)
	evol := NewDECurrentToRand1(f, maxGen, maxFES, st, hof, evaluator)
	evol.Init(pop)
	evol.Run()
